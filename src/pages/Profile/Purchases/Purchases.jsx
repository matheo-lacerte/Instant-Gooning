    ...
    async function load() {
      ...
      try {
        ...
        if (res.ok) setPurchases(Array.isArray(data) ? data : []);
          ...
          setError(data.error || "Erreur lors de la récupération des achats");
      } catch (e) {
          ...
          {purchases.map((p, idx) => (
            ...
            >
              ...
              >
                ...
                <div style={{ color: "var(--muted, #9ca3af)", fontSize: 12 }}>
                  {new Date(p.created_at).toLocaleString?.() || p.created_at}
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                {p.order_id && (
                  <div>
                    <strong>Commande:</strong> {p.order_id}
                  </div>
                )}
              </div>
            </div>
